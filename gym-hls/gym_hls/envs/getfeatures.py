...
def run_stats(bc_code, path="."):
    ...
    Returns:
	    Returns a list of all the features extracted from the bitcode file, which is the compiled version of program we need to run, that exist in our features list (that we assigned in this class). 
    """
...
def parse_static_features_str(out):
    ...
    Returns:
	    Returns a list of all the features extracted from the bitcode file, which is the compiled version of program we need to run, that exist in our features list(This is a helper function for run_stats and both functions return the same list).
