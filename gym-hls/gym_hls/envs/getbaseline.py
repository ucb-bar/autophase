...
def getbaseline(bm_name='chstone', num_pgms=None, clang_opt=False):
  ...
  Args:
    ...
    Clang_opt (bool, optional): clang_opt should be set to True if you want to calculate the reward using the clang option. 
      Otherwise, clang_opt should be set to False.

  Raises:
    The function terminates if the bm_name is not chstone or random.     

  Returns:
	  Prints the name of each program (benchmarks), wth its o0 and o3 cycles reward (the reward is higher if the compile time 
    of this program is less than the other programs), and o0 and o3 compile time.
    """
